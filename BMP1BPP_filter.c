...
**
** We use C-style comments to indicate the code from the orginal filter
** and C++-style to indicate the Bevara filter framework code
** 
***************************************************************/

#include <gpac/filters.h>

/* Since this is a simple filter, we will not use a separate header file 
   we simply insert all of our defintions, declarations, etc. up front */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <math.h>
...
/* these are the subfunctions for our BMP 1BPP filter */
...
/**************************************************************
  This handles the 1BPP format data decode 
**************************************************************/
int dec1(const char* bmp_data)
{
...
static GF_Err BMP1BPP_filter_process(GF_Filter *filter)
{
	...
	/* insert the BMP 1BPP filter code here, make sure we link the data_src to our local input data */
	...
	else /* we shouldn't reach here if did earlier sanity check */
			return GF_NOT_SUPPORTED;
	...
	/* set the output image WIDTH, HEIGHT and STRIDE from local functions, assuming RGB format */
...
static GF_Err BMP1BPP_filter_config_input(GF_Filter *filter, GF_FilterPid *pid, Bool is_remove)
{
	...
	if (stack->src_pid==pid) {
		...
		//update of caps, check everything is fine
			...
			// if our capabilities don't work, then we are done
	...
	// specify our output format and our filter name
...
const GF_FilterRegister BMP1BPPRegister = {
	...
	GF_FS_SET_HELP("Simple demonstration Accessor filter for BMP 1BPP images.")
